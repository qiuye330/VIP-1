...
# 获取有效 extra.sh 链接
get_valid_extra() {
    extra_list=(https://github.com/qiuye330/VIP-1/blob/main/Tasks/qlrepo/extra.sh)
...
# 获取有效 code.sh 链接
get_valid_code() {
    code_list=(https://github.com/qiuye330/VIP-1/blob/main/Scripts/sh/Helpcode2.8/code.sh)
...
get_valid_task_before() {
    task_before_list=(https://github.com/qiuye330/VIP-1/blob/main/Scripts/sh/Helpcode2.8/task_before.sh)
...
add_curl_sample() {
    if [ "$(grep -c "config.sample.sh" /ql/config/crontab.list)" != 0 ]; then
        ...
        # 获取token
        token=$(cat /ql/config/auth.json | jq --raw-output .token)
        curl -s -H 'Accept: application/json' -H "Authorization: Bearer $token" -H 'Content-Type: application/json;charset=UTF-8' -H 'Accept-Language: zh-CN,zh;q=0.9' --data-binary '{"name":"自动更新模板","command":"curl -L https://github.com/qiuye330/VIP-1/blob/main/Scripts/sh/Helpcode2.8/config_sample.sh -o /ql/sample/config.sample.sh && cp -rf /ql/sample/config.sample.sh /ql/config","schedule":"45 6,18 * * *"}' --compressed 'http://127.0.0.1:5700/api/crons?t=1627380635389'
    fi
}
run_curl_sample() {
